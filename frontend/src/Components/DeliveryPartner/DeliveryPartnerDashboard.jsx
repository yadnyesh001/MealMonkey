...
const DeliveryPartnerDashboard = () => {
  ...
  useEffect(() => {
    const fetchTodaysOrders = async () => {
      try {
        const response = await axiosInstance.get('/orders/today'); // API to fetch today's orders
  ...
  const handleAcceptOrder = async (orderId) => {
    try {
      await axiosInstance.put(`/orders/${orderId}/status`); // API to update order status
  ...
  return (
    <div className="flex">
      <aside className="w-1/4 h-screen bg-orange-400 to-orange-200">
        {/* Add profile and navigation details here */}
      </aside>
      ...
      <main className="ml-1/4 w-3/4 p-8">
        ...
        <div className="mt-6 bg-white p-6 rounded-lg shadow-lg h-96 overflow-y-auto">
          {pendingOrders.length > 0 ? (
            ...
            <p className="text-lg text-gray-600 text-center">No pending orders for today.</p>
          )}
        </div>
      </main>
    </div>
